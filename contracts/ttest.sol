...
contract SupplyChain {
    ...
    modifier onlyByManufacturer(uint256 _medicineID) {
        uint256 _id = findMAN(msg.sender);
        require(_id > 0, "Only a registered manufacturer can perform this action");
        require(MedicineStock[_medicineID].MANid == _id, "This action can only be performed by the assigned manufacturer");
        _;
    }
    ...
    struct medicine {
        ...
        uint256 RETid;
        STAGE stage;
        uint256 manufacturingDate; // Timestamp of manufacturing
    ...
    function addMedicine(string memory _name, string memory _description) public onlyByOwner() {
        ...
        medicineCtr++;
        MedicineStock[medicineCtr] = medicine(medicineCtr, _name, _description, 0, 0, 0, 0, STAGE.Init, 0, 0);
    }
    ...
    function setManufacturingAndExpiryDate(uint256 _medicineID, uint256 _manufacturingDate, uint256 _expiryDate) public onlyByManufacturer(_medicineID) {
        require(MedicineStock[_medicineID].stage == STAGE.Manufacture, "Medicine must be in manufacturing stage.");
        MedicineStock[_medicineID].manufacturingDate = _manufacturingDate;
